...
namespace FluentAssertionsToShouldly
{
    public class Converter
    {
        public static readonly Dictionary<string, string> AssertionReplacements = new()
        {
            ...
            // Handle Dictionary with strict ordering
            ...
            // Handle BeEquivalentTo with WithoutStrictOrdering
            { @"\.Should\(\)(?:\s*\n*\s*|\s+)\.BeEquivalentTo\((.*?),\s*options\s*=>\s*options\.WithoutStrictOrdering\(\)\s*\)", ".ShouldBeEquivalentTo($1)" },
            
            // Generic BeEquivalentTo pattern 
            { @"\.Should\(\)\s*\n*\s*\.BeEquivalentTo\(([^,\)]+)\)", ".ShouldBeEquivalentTo($1)" },

            // Handle exception with message assertions - most specific first
            { @"[\s\n]*(.*?)[\s\n]*\.Invoking\((.*?)\s*=>\s*\2\[(.*?)\]\)[\s\n]*\.Should\(\)[\s\n]*\.Throw<([^>]+)>\(\)[\s\n]*\.WithMessage\((.*?)\)", 
              @"
            var exception = Should.Throw<$4>(
                () => $1[$3]);
